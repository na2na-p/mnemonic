...
class TestPluginInfo:
    ...
    def test_plugin_info_is_frozen(self) -> None:
        ...
        info = PluginInfo(
            name="extrans",
            paths={
                "arm64-v8a": Path("/cache/plugins/arm64-v8a/libextrans.so"),
            },
        )
...
class TestPluginsInfo:
    """PluginsInfoデータクラスのテスト"""

    def test_plugins_info_is_frozen(self) -> None:
        """正常系: PluginsInfoは不変オブジェクト"""
        info = PluginsInfo(
            plugins={
                "extrans": PluginInfo(
                    name="extrans",
                    paths={"arm64-v8a": Path("/cache/plugins/arm64-v8a/libextrans.so")},
                ),
            }
        )

        with pytest.raises(AttributeError):
            info.plugins = {}  # type: ignore[misc]

    def test_get_all_paths_for_abi(self) -> None:
        """正常系: 指定ABIの全プラグインパスを取得する"""
        info = PluginsInfo(
            plugins={
                "extrans": PluginInfo(
                    name="extrans",
                    paths={
                        "arm64-v8a": Path("/cache/plugins/arm64-v8a/libextrans.so"),
                        "armeabi-v7a": Path("/cache/plugins/armeabi-v7a/libextrans.so"),
                    },
                ),
                "wuvorbis": PluginInfo(
                    name="wuvorbis",
                    paths={
                        "arm64-v8a": Path("/cache/plugins/arm64-v8a/libwuvorbis.so"),
                        "armeabi-v7a": Path("/cache/plugins/armeabi-v7a/libwuvorbis.so"),
                    },
                ),
            }
        )

        paths = info.get_all_paths_for_abi("arm64-v8a")
        assert len(paths) == 2
        assert "extrans" in paths
        assert "wuvorbis" in paths
...
class TestPluginFetcher:
    ...
    @pytest.fixture
    def single_plugin_config(self) -> list[PluginConfig]:
        """テスト用の単一プラグイン設定"""
        return [
            PluginConfig(
                name="extrans",
                ...
                url_template="https://example.com/{abi}/plugin.zip",
            )
        ]
    ...
    async def test_get_plugin_caching_behavior(
        ...
        single_plugin_config: list[PluginConfig],
        ...
        if cached_plugin_exists:
            for abi in SUPPORTED_ABIS:
                ...
                plugin_file = abi_dir / "libextrans.so"
        ...
        fetcher = PluginFetcher(
            ...
            plugin_configs=single_plugin_config,
        )
    ...
    def test_is_cache_valid_returns_true_when_all_plugins_exist(
        ...
        fetcher = PluginFetcher(cache_dir=temp_cache_dir, plugin_configs=single_plugin_config)
